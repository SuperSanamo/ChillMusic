        ...
        if self.config._spotify:
            try:
                ...
                if not self.spotify.token:
                    log.warning('Where tf is my token. We disabling this shit.')
                    self.config._spotify = False
                else:
                    log.info('Your Spotify Is GOOD To Go :)')
            except exceptions.SpotifyError as e:
                log.warning('Could not connect to this spoify shit. Check your info and shit that you entered. Details: {0}. Continuing anyway in 5 seconds...'.format(e))
                self.config._spotify = False
    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("Only the owner can use this command. GTFO", expire_in=30)
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if str(orig_msg.author.id) in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError("Only dev users can use this command. GTFO", expire_in=30)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.members and author_perms.skip_when_absent:
                ...
                newmsg = 'In the channel:`%s`: `%s` is playing which was added by `%s`' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            ...
            while player.autoplaylist:
                ...
                if info.get('entries', None):  # or .get('_type', '') == 'playlist'
                    log.debug("Playlist found but can't be played buddy :) Sorry. Jk fk u.")
                ...
                try:
                    ...
                    log.error("This fucking song can't be added : {}".format(e))
            ...
            if not self.autoplaylist:
                # TODO: When I add playlist expansion, make sure that's not happening during this check
                log.warning("No songs found that this nigga can play in the autoplaylist, we turning this shit off right now.")
                self.config.auto_playlist = False
...
#######################################################################################################################
    ...
    async def safe_send_message(self, dest, content, **kwargs):
        ...
        try:
            ...
            lfunc("Bitch you can't send message to \"%s\", no damn permissions buddy.", dest.name)
            ...
            if len(content) > DISCORD_MSG_CHAR_LIMIT:
                lfunc("Message is over the message size limit (%s) shorten your shit down please. <3", DISCORD_MSG_CHAR_LIMIT)
            else:
    ...
    async def safe_delete_message(self, message, *, quiet=False):
        ...
        try:
            ...
            lfunc("Bitch you can't delete message \"{}\", no damn permissions buddy.".format(message.clean_content))

        except discord.NotFound:
            lfunc("Bitch you can't delete message \"{}\", I can't fucking find the message.".format(message.clean_content))
    ...
    async def safe_edit_message(self, message, new, *, send_if_fail=False, quiet=False):
        ...
        try:
            ...
            lfunc("Slut, YOU CAN'T FUCKING EDIT THE DAMN MESSAGE \"{}\", I just can't fucking find it? Tf?".format(message.clean_content))
            if send_if_fail:
    ...
    async def send_typing(self, destination):
        try:
            ...
            log.warning("Ffs I can't send typing to {}, not sure why? perms?".format(destination))
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Chill Music Bot By SanamoSuper ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/rINFyQb.png')
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help ask an administrator / developer of the bot.').format(prefix)
        if not self.is_all:
